...
def main(
    ...
    if run_etl:
        run_args_etl: dict[str, Any] = {}
        pipeline_args["config_path"] = root_dir / "configs" / etl_config_filename  # type: ignore
        assert pipeline_args[  # type: ignore
            "config_path"
        ].exists(), f"Config file not found: {pipeline_args['config_path']}"
        pipeline_args[
            "run_name"
        ] = f"digital_data_etl_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"  # type: ignore
